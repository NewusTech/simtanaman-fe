...
import { SubmissionPlant } from "@/types/planting/submissionPlant";
import { fetchSubmissionPlantData } from "@/lib/planting/submissionPlantFetching";
...
export default function SubmissionPlantPage() {
  ...
  const [listPengajuan, setListPengajuan] = useState<SubmissionPlant[]>([]);
  ...
  const fetchPage = useCallback(
    async (page: number) => {
      ...
      setLoading(true);
      const data = await fetchSubmissionPlantData(page, String(token));
        ...
        <TableBody>
          {loading ? (
            ...
            <TableRow key={listPengajuan.indexOf(value)}>
              ...
              <TableCell className="font-medium">{new Date(value.createdAt).toLocaleDateString("id-ID", {
                        day: "2-digit",
                        month: "2-digit",
                        year: "numeric",
                      })}</TableCell>
              <TableCell>disributor</TableCell>
              <TableCell className="text-center">
                {value.tanaman?.name || "Tidak Diketahui"}
              </TableCell>
              <TableCell className="text-center">
                {value.jumlah}
              </TableCell>
