...
export default function FarmerPage() {
  ...
  const handleOpenModal = (id: number) => {
    setIsOpen(true);
    setStatus("Pengguna Petani");
    setId(id);
  };

  const handleAddAdmin = (slug: string, params?: Object) => {
    router.push("/home/management/farmer/" + slug + (params ? `?${new URLSearchParams(params as any)}` : ""));
  };
  ...
  return (
    <div className="bg-white p-4 rounded-md shadow-md font-poppins">
      ...
      <div className="mt-4 flex items-center justify-between w-full gap-4">
        ...
        <button
          onClick={() => handleAddAdmin("Tambah Petani")}
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent className="bg-white shadow-md rounded-md absolute left-[-110px]">
                      <DropdownMenuItem
                        ...
                        onClick={() => handleAddAdmin("Detail Petani", { id: value.id })}
                      >
                        Detail
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        className="cursor-pointer"
                        onClick={() => handleAddAdmin("Edit Petani", { id: value.id })}
                      >
                        Edit
                      </DropdownMenuItem>
                      <DropdownMenuItem
